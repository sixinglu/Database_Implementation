...
Page* header_ptr;
MINIBASE_BM->pinPage(headerpageID, (Page* &)header_ptr, 0 );
memcpy(&headerpage,header_ptr,sizeof(HeaderPage));
MINIBASE_BM->unpinPage(headerpageID, 1, 1);
		returnStatus =  OK;
	}
...
Page* header_ptr;
MINIBASE_BM->newPage(headerpageID, (Page* &)header_ptr);
		// allocate space
...
//memcpy(header_ptr,&headerpage,sizeof(HeaderPage));
...
//MINIBASE_BM->unpinPage(headerpageID, 1, 1);
	//if(destroyed == false)
	//	MINIBASE_DB->add_file_entry(headerpage.filename,headerpageID);
