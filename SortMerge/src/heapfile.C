...
int HeapFile::getRecCnt()
{
	...
	while(iterate_dirPage != INVALID_PAGE){   // when reach Pid=-1 means the end of the linklist
		...
		if(status!=OK){
            MINIBASE_BM->unpinPage(iterate_dirPage,1,0);
		...
		status = MINIBASE_BM->unpinPage(iterate_dirPage,1,0);             // not dirty, unpin dir page
		if(status!=OK){
			return MINIBASE_FIRST_ERROR(HEAPFILE, status);
		}
...
Status HeapFile::insertRecord(char *recPtr, int recLen, RID& outRid)
{
	...
	while(iterate_dirPage != INVALID_PAGE){   // when reach Pid=-1 means the end of the linklist
		...
		do{
			...
			if( dirinfo->availspace > recLen){  // avaiable space, read out the real record page
				HFPage* currpage;
				status = MINIBASE_BM->pinPage( dirinfo->pageId, (Page* &)currpage,0 );  // the page contains real record
				//cout<<"pin: "<<dirinfo->pageId<<endl;

				if(status!=OK){
					MINIBASE_BM->unpinPage(dirinfo->pageId,1,0);
				...
				status = MINIBASE_BM->unpinPage(dirinfo->pageId,1,0);              // dirty, unpin record page
				//cout<<"unpin: "<<dirinfo->pageId<<endl;

				if(status!=OK){
					return MINIBASE_FIRST_ERROR(HEAPFILE, status);
				}
				status = MINIBASE_BM->unpinPage(iterate_dirPage,1,0);                   // dirty, unpin dir page
				//cout<<"unpin: "<<iterate_dirPage<<endl;
		...
		if(successFlag==true){
			...
			status = MINIBASE_BM->pinPage(newdpinfop.pageId, (Page *&)readPage,0);
			...
			HFPage* prevpage;
			status = MINIBASE_BM->pinPage( dirinfo->pageId, (Page* &)prevpage,0);
			//cout<<"pin: "<<dirinfo->pageId<<endl;
			...
			//printf("insert pno %d sno %d to dir pno %d sno %d\n",outRid.pageNo,outRid.slotNo,recordinPage.pageNo,recordinPage.slotNo);
			if(status!=OK){
                MINIBASE_BM->unpinPage(newdpinfop.pageId,1,0);
			...
			newdpinfop.recct = 1;
			status = MINIBASE_BM->unpinPage(newdpinfop.pageId,1,0);   // dirty
			//cout<<"unpin: "<<newdpinfop.pageId<<endl;
	...
	if(successFlag==false){
		...
		// insert record
		HFPage* recordPage;
		MINIBASE_BM->pinPage(allocDataPageRid.pageNo,(Page* &)recordPage,0);
		//cout<<"pin: "<<allocDataPageRid.pageNo<<endl;

		status = recordPage->insertRecord(recPtr, recLen, outRid);
		if(status!=OK){
            MINIBASE_BM->unpinPage(allocDataPageRid.pageNo,1,0);
		...
		status = MINIBASE_BM->unpinPage(allocDataPageRid.pageNo,1,0);
		//cout<<"unpin: "<<allocDataPageRid.pageNo<<endl;
		...
		HFPage* currDirPage;
		RID NewDirRid;
		status = MINIBASE_BM->pinPage( NewDirPageId, (Page* &)currDirPage ,0);
		...
		HFPage* prevDirPage;
		status = MINIBASE_BM->pinPage(prev,(Page* &)prevDirPage,0);
		//cout<<"pin: "<<prev<<endl;
		...
		status = MINIBASE_BM->unpinPage(prev, 1,0);
		...
		status = MINIBASE_BM->unpinPage(NewDirPageId,1,0);
...
Status HeapFile::deleteRecord (const RID& rid)
{
	...
	if(rpdatapage!=NULL){  // find the record
		status = MINIBASE_BM->pinPage(rpDirPageId, (Page*&)rpdirpage,0);     // pin here again
		if(status!=OK){
			return MINIBASE_FIRST_ERROR(HEAPFILE, status);
		}
		...
		if(status!=OK){
            MINIBASE_BM->unpinPage(rpDirPageId,1,0);
            MINIBASE_BM->unpinPage(rpDataPageId,1,0);
			return MINIBASE_FIRST_ERROR(HEAPFILE, status);
		}
		...
		status = MINIBASE_BM->unpinPage(rpDataPageId,1,0);   // unpin back here
		if(status!=OK){
			return MINIBASE_FIRST_ERROR(HEAPFILE, status);
		}

		status = MINIBASE_BM->unpinPage(rpDirPageId,1,0);
		if(status!=OK){
			return MINIBASE_FIRST_ERROR(HEAPFILE, status);
		}
		//        }
...
//
		status = MINIBASE_BM->unpinPage(rpDataPageId,1,0);
		if(status!=OK){
			return MINIBASE_FIRST_ERROR(HEAPFILE, status);
		}
...
Status HeapFile::newDataPage(DataPageInfo *dpinfop)
{
	...
	status = MINIBASE_BM->unpinPage(recordID, 1,0);
	if(status!=OK){
		return MINIBASE_FIRST_ERROR(HEAPFILE, status);
	}
		...
		do{
			...
			do{
				...
				if(DataPageRid.slotNo==rid.slotNo){    // find the record
					...
					status = MINIBASE_BM->unpinPage(dirinfo.pageId,1,0);   // do not uppin here, still need to use
					if(status!=OK){
						return MINIBASE_FIRST_ERROR(HEAPFILE, status);
					}

					status = MINIBASE_BM->unpinPage(iterate_dirPage,1,0);
					if(status!=OK){
						return MINIBASE_FIRST_ERROR(HEAPFILE, status);
					}
		...
		RID &allocDataPageRid)
{
	...
	if(status!=OK){
        MINIBASE_BM->unpinPage(allocDataPageRid.pageNo,1,0);
        MINIBASE_BM->unpinPage(allocDirPageId,1,0);
		return MINIBASE_FIRST_ERROR(HEAPFILE, status);
	}
	...
	status = MINIBASE_BM->unpinPage(allocDirPageId, 1,0);
	if(status!=OK){
		return MINIBASE_FIRST_ERROR(HEAPFILE, status);
	}
